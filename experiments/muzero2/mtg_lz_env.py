"""LightZero env adapter over ``mtgenv_gym.MtgEnv`` (muzero2 workstream).

Ported from ``experiments/stochastic_muzero/swine_lightzero_env.py`` (READ-ONLY reference) — same
mapping, registered under a fresh name ``mtg_lz`` and deck-agnostic via ``env.deck``:
  ...
  * to_play      -> ``-1``: single-agent stochastic MDP (opponent + card draws folded into env
                    stochasticity). See stochastic_muzero/README "why single-agent".

Dense reward shaping (``reward_shaping`` coef, CONSTANT — no annealing) uses the SAME card-dominant
PBRS potential the PPO baseline uses (``batched_selfplay._phi_batch``). Policy-invariant
(``Phi(terminal)=0``); ``eval_episode_return`` is kept RAW (±1) so evaluated win/loss is never shaped.
"""
...
@ENV_REGISTRY.register('mtg_lz')
class MtgLzEnv(BaseEnv):
    ...
    config = dict(
        ...
        max_decisions=3000,
        agent_seat=0,
        # Potential-based reward-shaping coefficient (0.0 = OFF = pure sparse ±1). >0 adds
        # F = gamma*Phi(s') - Phi(s) with the card-dominant Phi used by the PPO training.
        reward_shaping=0.0,
        shaping_gamma=0.997,
    )
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self._deck = cfg.get('deck', 'heralds')
    ...
    def reset(self):
        ...
        self._final_eval_reward = 0.0
        if self._shaping:
            self._prev_phi = self._phi(obs_dict)
    ...
    def step(self, action):
        ...
        self._final_eval_reward += float(reward)   # RAW ±1 — eval win/loss is never shaped

        train_reward = float(reward)
        if self._shaping:
            if done:
                f = -self._prev_phi                 # Phi(terminal) := 0
            else:
        ...
        rew = to_ndarray(train_reward).astype(np.float32)  # 0-d scalar (buffer pads with np.array(0.))
        out_info = {}
        if done:
    ...
    def __repr__(self) -> str:
        return f"MtgLzEnv(deck={self._deck}, opponent={self._opponent}, obs_dim={self._obs_dim})"
