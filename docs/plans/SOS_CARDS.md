...
- **sos-bonus-1 continued (2026-07-06): 48/65, 948 mtg-core green.** Lead approved the C-subsystem sketches (3 notes) + all remaining.
  ...
  - **★ REMAINING 17:** Spree (Requisition Raid, Return the Favor — `Ability::Spree` + `Effect::ChangeTarget`), Roles (Monstrous Rage, Royal Treatment — `Effect::CreateRoleToken`), then singles Protection/Infect/Suspend/Convoke/Redirect/Split-second; + Berserk (attacked-this-turn), Veil of Summer (3 mechanisms), Culling Ritual (ManaSpec churn — deferred), Daze/Force of Will (alt-cast).
