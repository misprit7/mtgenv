...
- **sos-bonus-1 continued (2026-07-06): 46/65, 943 mtg-core green.** Lead-greenlit moderate caps shipped:
  ...
  - **★ REMAINING 19:** (see below — lead approved all)
- **sos-bonus-1 continued (2026-07-06): 48/65, 948 mtg-core green.** Lead approved the C-subsystem sketches (3 notes) + all remaining.
  - **OVERLOAD subsystem** (`Ability::Overload{cost}` + `CastVariant::Overload` + `Object.overloaded`) → **Cyclonic Rift, Winds of Abandon**. The card writes ONE single-target effect; `overload_rewrite(effect)` (priority.rs) derives the "each" version — `Target(spec)`→`ForEach{selector, body}` with `retarget_to_each` mapping `Target`→`Each` and `ControllerOfTarget`→**`ControllerOfEach`** (new `PlayerRef`, reads `foreach_current`'s controller) so Winds' per-exiled land-search runs per creature per its controller. The ForEach **span** comes from the target filter's controller restriction (`chooser_from_control_filter`: "you don't control"→`EachOpponent`) because `count_filter_matches` is ctx-free (`ControlledBy`→true). Overloaded cast chooses **no targets** (tested: empty `obj.targets`, no `Targeted` events, CR 702.96b). Offered from hand independent of the single-target legality gate.
  - **★ REMAINING 17 — build spec for the next agent (lead's 3 notes are in the sketch section above):**
    - **② Spree** (Requisition Raid, Return the Favor): `Mode` has **67 literals** — do NOT add a `cost` field to it (worse churn than the reverted ManaSpec `one_of`). Instead a bespoke `Effect::Spree{ modes: Vec<SpreeMode{ cost: ManaCost, label, effect }> }` (new struct, no churn) + cast-pipeline integration: choose ≥1 modes (601.2b), total cost = base + Σ chosen modes' `cost`, pay it, resolve each chosen effect. Mirror the Overload cast wiring (offer/cost/effect). `Effect::ChangeTarget` (mode 2) validates the new target vs the spell's own `TargetSpec` (filter+zone), offered/resolves only when a legal alternative exists; impossible retarget leaves the target unchanged (per lead note ②). Mode 1 = `CopySpellOnStack` (exists).
    - **③ Roles** (Monstrous Rage, Royal Treatment): resolution-time `Effect::CreateRoleToken{ role, attach_to }` — mint a registered Role **Aura token** def (model on `mir/pacifism`: `Ability::Static{ ModifyPT(+1/+1), affects: attached_host() }` + a keyword static). Monster Role = +1/+1 & `GrantKeyword(Trample)` (easy). **Royal Role = +1/+1 & ward {1}** — bare `Keyword::Ward` carries no cost, so grant a **ward ABILITY** (`StaticContribution::GrantAbility` over a ward template in the 9800+ block, or a `ward_mana(1)` grant) rather than the keyword. Then `Attach` (exists) + sacrifice the controller's prior Role on that creature (lead note ③: newest survives, prior→graveyard at attach; ledger the CR 303.4k SBA-timing approximation; test that a Role token going to graveyard ceases to exist via the token SBA).
    - **Singles:** Protection-from (Akroma's Will mode 2), Infect (Triumph of the Hordes — poison SBA exists), Suspend (Living End), Convoke (Return to the Ranks — sketch against §2.6), Redirect (Deflecting Palm — `Rewrite::Redirect` is stubbed), Split-second+can't-lose (Angel's Grace).
