...
- **▶ SOS FULL-SET scope: 256/271 authored (94%, 253 faithful · 3 tracked-partial), 830 mtg-core tests green (2026-07-05,
  sos-cards-18).** 8 fully-faithful cards + 7 reusable caps; the **clean cap-blocked tail is now EXHAUSTED** (15 unauthored: 11
  subsystem-scale caps/one-offs + 4 parked). Also shipped **Skycoach Waypoint** (`Effect::SetPrepared`), **Biblioplex
  Tomekeeper** (modal triggered-ability support), **Mana Sculpt** (delayed-mana cap). **Burrog Barrage**
...
- **▶ SOS (prior — sos-cards-17 FIRST WAVE): 244/271, 794 green.** **THE SPELL-COPY CONSUMERS + THE LKI-COUNTER,
  ...
  scratch); **LKI counter-count** (`Lki` snapshots the counter bag at death; `CountersOnSelf` falls back to it off-battlefield —
  BOTH eval paths); **`Effect::SetNoMaxHandSize`**; **`Ability::ExileOnResolve`** (self-exiling I/S); **`Effect::
  GrantChosenKeyword`** ("your choice of X or Y"); **`Effect::PutDiscardedOntoBattlefield`**; **`Effect::MoveCounters`**;
  **`MayPayCost`-with-`{X}`** (announce/pay X, thread to reward as `ValueExpr::X`, targeted reward collected as a normal target);
  **fixed `interpret_search` dynamic-filter resolution**. Tracked-partials now 3 (Ral, Wildgrowth, Hydro-Channeler). **Clean-
  compose tail EXHAUSTED** — the 27 unauthored all need a new cap or lead sketch (see SOS_CARDS.md census). Highest-yield next
  cap = **target-path dynamic-filter resolution** (unlocks Moseo + Sundering Archaic). 0 Native hatches.
- **▶ SOS (prior — sos-cards-16): 234/271 authored (86%), 764 mtg-core tests green (2026-07-05).** **ALL 5 COLLEGE ELDER
  ...
  (702.83 — exile-until-nonland-cheaper + may-free-cast + `state.rng` random-bottom); **the MIRACLE subsystem** (702.94,
  lead-approved plan A — `Ability::Miracle`/`GrantMiracle` + `miracle_cost` two-origin check + first-draw capture in `draw()` +
  `StackObjectKind::MiracleWindow` + `CastVariant::Miracle`). Dragons: **Prismari** (Storm), **Silverquill** (Casualty),
  **Witherbloom** (Affinity), **Quandrix** (Cascade), **Lorehold** (Miracle — grants miracle {2} to your I/S + opp-upkeep loot).
  Plus **Lumaret's Favor** + **Social Snub** (SelfCast+CopySpellOnStack consumers). Still unblocked for the next agent:
  target-spell copy consumers (Choreographed Sparks); medium caps (Increment, Ennis exile-tracker, NoMaxHandSize, Moseo,
  LKI-counter). 0 Native hatches.
...
- **▶ SOS (prior — sos-cards-9): ~163 authored, 602 mtg-core
  ...
  cost reduction (Diary of Dreams)** — the graveyard-recursion/graveyard-ability vein is cleared and the S12
  cost-reduction mechanism now covers cast + activation costs. No-rewind recorded as a pragmatic economy
  (transactional pending-cast = the sanctioned evolution, GRE model) in WHITEBOARD_MODEL §2.6. **Next: the big
  three (Planeswalkers first).** Each subsystem built as the general CR capability, not the minimal hack (the big three still
